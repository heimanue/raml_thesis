...
the factorization used throughout the book, this joint distribution is written as
\begin{equation}
p(x,y)=p(y\mid x)p(x).
\tag{6.1}
\label{eq:book-6-1}
...
The flow-based hybrid model uses a slightly different weighting scheme: only the
density term is scaled.
\begin{equation}
\ell(x,y;\lambda)
=
...
for ordinal integer values because nearby integers remain related.
