...
\begin{abstract}
Hybrid modeling combines predictive and generative modeling by describing the
joint distribution as \(p(x,y)=p(y\mid x)p(x)\). This factorization addresses a
weakness of purely discriminative classifiers: a model for \(p(y\mid x)\) may
assign a confident class prediction to an input that lies far away from the
training distribution. A model for \(p(x)\), in contrast, provides information
about whether the input itself is plausible. This report explains the motivation,
mathematical background, and main construction of hybrid models with invertible
feature representations. Normalizing flows provide the key building block because
they transform data into latent variables through bijective maps and allow exact
likelihood evaluation through the change of variables formula. The report follows
the derivation from separate predictive and generative models to shared
parameterization, explains the scale mismatch between classification and density
terms, and introduces the weighted hybrid objective. It then presents Hybrid
Integer Discrete Flows as a concrete model for integer-valued data and discusses
the resulting benefits, limitations, and open questions.
...
This limitation matters because confidence is not the same as distributional
awareness.
...
\caption{An example of adding noise to an almost perfectly classified image
that results in a shift of predicted label. Since a discriminative model must
always assign probability mass to some class, it provides no signal that the
perturbed input is unusual. Reprinted from~\cite{tomczak2024deep}, Fig.~1.1.}
\label{fig:confidence-flip}
\end{figure}

Hybrid modeling addresses this problem by modeling the joint distribution of
inputs and labels. Following the book's hybrid modeling chapter~\cite{tomczak2024deep},
the factorization used throughout this report is
\begin{equation}
p(x,y)=p(y\mid x)p(x).
...
is therefore best understood as distribution-aware prediction, not as pure
generation with labels attached.
...
\caption{Comparison between a purely discriminative classifier and a
...
out-of-distribution. Reprinted from~\cite{tomczak2024deep}, Fig.~1.2.}
...
optimized for predictive performance, so its representation is shaped by label
separation rather than by the full structure of the input distribution.
...
The central technical question is how to make the two factors interact during
learning. If the classifier and density model are trained independently, their
scores can be combined, but their representations do not influence each other.
The more interesting case is shared representation learning, where predictive
and generative gradients shape the same parameters.
...
Invertibility is crucial. In a standard feed-forward classifier, a
...
formula:
\begin{equation}
p(x)=\pi(z)\left|J_f(z)\right|^{-1}.
\label{eq:flow-likelihood}
\end{equation}
Here \(z=f^{-1}(x)\), and \(J_f(z)\) denotes the Jacobian matrix of the
transformation from latent space to data space at that latent point. The vertical
bars denote the absolute determinant of this matrix. The book sometimes writes
this dependency more compactly as \(|J_f(x)|\), but the transformation \(f\)
itself acts on the latent point corresponding to the observed input. The
base-density term measures whether the latent representation is plausible under
\(\pi\). The determinant term corrects for the local volume change induced by
the transformation.

The Jacobian determinant has a geometric interpretation. If the mapping
...
compress space. The change of variables formula is therefore what allows an
invertible neural network, together with a base distribution, to define a valid
probability density over inputs.

Flows become especially useful when \(f\) is built as a composition of
invertible functions. If \(f=f_K\circ\cdots\circ f_1\) and
\(z_i=f_i(z_{i-1})\) with \(z_K=x\), then the log-likelihood can be written as a
base log-density minus a sum of log-Jacobian terms:
\begin{equation}
\ln p(x)
=
...
classifier-only representation. At the same time, the predictive and generative
objectives can pull the shared representation in different directions.
...
The previous section introduced the two ingredients needed for hybrid modeling:
a predictive component \(p(y\mid x)\) and a tractable density model \(p(x)\).
This section explains how these ingredients are combined into a single training
objective. The key issue is not the factorization itself, which follows directly
from probability theory, but the interaction between representation sharing,
gradient flow, and scale. The derivation therefore proceeds in three steps: a
naive combination with separate parameters, a shared parameterization, and a
weighted objective that controls the balance between classification and density
modeling.

\subsection{Naive Composition and Shared Parameterization}

The natural starting point is the joint distribution of inputs and labels. In
the factorization used throughout the book~\cite{tomczak2024deep}, this joint
...
\label{eq:book-6-1}
\end{equation}
It states that the model should assign high joint probability only
...
The zero terms in Eqs.~\eqref{eq:book-6-3} and~\eqref{eq:book-6-4} are the key
diagnostic: the two distributions are connected only by addition of their scores,
not by shared learning. The naive construction therefore misses the main promise
of hybrid modeling, namely that label information and input-density information
can shape a common representation.
...
\begin{figure}
\centering
\includegraphics[width=.65\textwidth]{fig_6_1_naive_separate_models.png}
...
This parameter sharing turns the joint model into a genuine hybrid architecture.
The parameters \(\alpha\) and \(\beta\) can specialize the predictive and
generative heads, but the shared part \(\gamma\) is optimized by both terms.
This design is useful only if the two objectives can be balanced; otherwise, the
larger term can dominate the shared representation.
...
The book makes this point with a binary example~\cite{tomczak2024deep}. Suppose
first that \(y\) is a binary label and the model assigns probability \(0.5\) to
both classes. Then
\[
...
Since both terms now depend on the shared parameters \(\gamma\), the update of
the common representation receives contributions from both the classification
term and the density term. If the density contribution is numerically much
larger, the shared representation may become more useful for density modeling
than for classification. This behavior can hurt the predictive component, even
though the architecture shares parameters in a principled way.
...
model~\cite{tomczak2024deep}. It is a useful training rule, but it gives up some
of the elegance of ordinary likelihood maximization.

This observation is important for interpreting the method. The problem is not
that Eq.~\eqref{eq:book-6-8} cannot be optimized; the problem is that the
objective no longer has the same clean likelihood interpretation as
Eq.~\eqref{eq:book-6-1}. Hybrid modeling therefore has to trade probabilistic
elegance against optimization control.

\subsection{Weighted Hybrid Objective with Invertible Features}

The flow-based hybrid formulation in the book uses a slightly different
weighting scheme: only the density term is scaled~\cite{tomczak2024deep}.
\begin{equation}
\ell(x,y;\lambda)
=
...
\label{eq:book-6-9}
\end{equation}
The parameter \(\lambda\) addresses the scale issue from the previous subsection
by controlling the relative influence of the generative component. It should be
understood as a balancing hyperparameter rather than as a parameter derived from
a normalized probabilistic model. The notation
\(\ell(x,y;\lambda)\) follows the book and denotes an objective to maximize; in
implementation, one would usually minimize its negative. When \(\lambda=0\), the
objective reduces to supervised classification. When \(\lambda=1\), it coincides
with the ordinary log-joint under Eq.~\eqref{eq:book-6-1}, provided that both
factors are normalized. Other values of \(\lambda\) are better interpreted as
controlled training criteria. Nalisnick et al. discuss related interpretations
in terms of robustness and Jacobian-based regularization~\cite{nalisnick2019hybrid},
but for the present purpose \(\lambda\) is best read as a practical balancing
parameter.
...
The objective in Eq.~\eqref{eq:book-6-9} becomes concrete once both terms are
parameterized. The predictive term is the log-probability assigned by the
classifier to the observed label \(y\), computed from the invertible features
\(z=f^{-1}(x)\) and a predictive head \(g\). The generative term is the flow
likelihood introduced in Section~\ref{sec:background}: the same latent
representation is evaluated under the base density \(\pi(z)\) and corrected by
the Jacobian term \(J_f\). Concretely, the first term becomes the selected
softmax log-probability \(\ln \theta_{y,g,f}(x)\), while the second term becomes
the flow log-likelihood
\(\ln \pi(z)-\ln |J_f(z)|\), with \(z=f^{-1}(x)\).

The key point is therefore structural rather than algebraic. The classifier does
not operate on a separate representation from the density model; both use the
same invertible backbone. The learned representation must make labels
predictable while still defining a tractable density for \(x\). This shared role
is what makes the model hybrid rather than merely a classifier with an auxiliary
score.

\subsection{Hybrid Integer Discrete Flows}

The flow-based objective above assumes a continuous density, which fits naturally
with standard normalizing flows but is not always the most faithful model for
data stored on a grid. An 8-bit image, for example, consists of integer pixel
values. This motivates a discrete version of the same hybrid idea based on
Integer Discrete Flows. Instead of requiring dequantization or other adjustments,
IDFs define bijective transformations directly on integer-valued
states~\cite{hoogeboom2019integer}.

This distinction is important because a continuous density gives probabilities
only after integration over regions, whereas a discrete model assigns probability
mass directly to individual states. Digital images and lossless compression
tasks care about exact stored integer values. IDFs keep the model aligned with
that representation instead of introducing a continuous relaxation as the main
object of modeling.

The key mathematical simplification is that a discrete bijection reassigns
probability mass between integer states. There is no infinitesimal volume element
to correct, so the continuous Jacobian term disappears. The density term is then
computed by evaluating the base probability mass at the latent state
\(z=f^{-1}(x)\). In the HybridIDF example, this base distribution is a
discretized logistic mass \(\operatorname{DL}(z\mid\mu,\nu)\), which is suitable
for ordinal integer values because nearby integers remain
related~\cite{hoogeboom2019integer,tomczak2024deep}.

The resulting HybridIDF objective is the discrete analogue of the continuous
hybrid objective from Chapter~6~\cite{tomczak2024deep}:
\begin{equation}
\ell(x,y;\lambda)
=
...
\label{eq:book-6-13}
\end{equation}
The classification log-probability stays in place, while the flow likelihood
term is replaced by a weighted discretized-logistic log-mass. The classifier still
uses features produced by the invertible transformation, and the density term
still trains the model to assign probability mass to the observed input. The
difference is that all transformations preserve integer-valued states.

The implementation details of IDFs are important but secondary for the hybrid
modeling argument. IDFs use integer-preserving coupling layers with rounded
translations. Rounding keeps the states discrete, but it is not differentiable,
so training relies on the straight-through estimator, which treats rounding
approximately as the identity during backpropagation~\cite{hoogeboom2019integer}.
This estimator makes gradient-based learning possible but introduces biased
gradients. Thus HybridIDF shows that the hybrid objective can be adapted to
discrete data, while also revealing the additional engineering compromises
required by integer-valued invertible models.
...
summarizes the types of evidence shown in the book's
Fig.~6.4~\cite{tomczak2024deep}: real images,
...
\caption{An example of outcomes after training: (a) Randomly selected real
images. (b) Unconditional generations from the HybridIDF. (c) An example of a
validation curve for the classification error. (d) An example of a validation
curve for the negative log-likelihood, i.e., $-\ln p(x)$. Reprinted
from~\cite{tomczak2024deep}, Fig.~6.4.}
...
family, and the evaluation setting. In high-dimensional data, likelihood can also
reflect low-level statistics that are not identical to semantic familiarity, so
density-based rejection must be validated for the specific task.
...
The book's outlook points to several ways forward~\cite{tomczak2024deep}. Hybrid
VAEs extend the same idea beyond flows, but they require a variational lower
bound instead of exact likelihood evaluation. Other architectures or learning
procedures may reduce the need for a manually tuned \(\lambda\). The choice of
factorization also remains important. The factorization \(p(y\mid x)p(x)\) is
well suited to reliable decision-making because it keeps input plausibility
explicit. A factorization \(p(x\mid y)p(y)\) may be more natural when
class-conditional generation is the main goal. Finally, HybridIDF illustrates
both the strength and the cost of integer-valued flows: they match discrete data
directly, but the rounding operation requires the straight-through estimator and
therefore biased gradients~\cite{hoogeboom2019integer}.
...
the objective when \(\lambda\neq 1\). This trade-off is not a minor implementation
detail; it is the point where the probabilistic model becomes a practical
learning criterion.
...
the missing question explicit: a classifier should not only decide which label an
input receives, but also whether the input is plausible under the distribution it
has learned.
