...
the ordinary log-joint under Eq.~\eqref{eq:book-6-1}, provided that both factors
are normalized. Other values of \(\lambda\) are better interpreted as controlled
training criteria, not automatically as log-likelihoods of new normalized joint
distributions. This distinction prevents a common misunderstanding: the weighted
objective is useful exactly because it trades probabilistic elegance for
...
The rest of the book's derivation instantiates Eq.~\eqref{eq:book-6-9} for a
classifier and a flow model. The predictive term is an ordinary categorical
classifier: for the observed label \(y\), the log-probability selects the
corresponding softmax value \(\theta_{y,g,f}(x)\). The subscripts indicate that
the class probability depends on both the invertible transformation \(f\) and the
predictive head \(g\). The generative term is the flow likelihood from
Section~\ref{sec:background}: the input is mapped to \(z=f^{-1}(x)\), evaluated
under the base density \(\pi(z)\), and corrected by the Jacobian term \(J_f\).

In this sense, the book's Eqs.~6.10--6.12 do not introduce a new conceptual
step. They write out the same weighted objective from Eq.~\eqref{eq:book-6-9}
for the concrete case of a categorical classifier and a continuous flow with a
Gaussian base density. The important message is that the classifier receives the
same invertible features that are used for likelihood evaluation. The learned
representation must therefore be predictive for \(y\) while still supporting a
valid density model for \(x\).
